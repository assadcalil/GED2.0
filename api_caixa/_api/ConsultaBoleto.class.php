<?php
    ...
    class ConsultaBoleto{
        ...
        private function _setConfigs($informacoes) {
            ...
            $requiredKeys = ['urlIntegracao', 'codigoCedente', 'nossoNumero', 'cnpj'];
            foreach ($requiredKeys as $key) {
                if (!isset($informacoes[$key])) {
                    throw new Exception("Parâmetro obrigatório não informado: {$key}");
                }
            }

            // Definir numeroAgencia com valor padrão se não existir
            if (!isset($informacoes['numeroAgencia'])) {
                $informacoes['numeroAgencia'] = '4105'; // Valor padrão
                // Alternativa: lançar uma exceção
                // throw new Exception("Número de agência não informado.");
            }
            ...
            // Corrigido para não dar echo, apenas gerar a estrutura XML
        ...
        private function _geraEstruturaXml(array $arrayDados) {
            ...
            // Não faz echo do XML, apenas armazena na propriedade
        }
