...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $verificaCodigo = $conn->prepare("SELECT * FROM empresas WHERE emp_code = ?");
        $verificaCodigo->execute([$_POST['emp_code']]);
        $empresaExistente = $verificaCodigo->fetch(PDO::FETCH_ASSOC);

        // Se a empresa já existir, definir mensagem de erro
        if ($empresaExistente) {
        ...
        // Adicionar o usuário que está fazendo o cadastro
        $empresaData['usuario'] = $_SESSION['user_name'] ?? 'Sistema';
        
        // Gerar nome da pasta para documentos
        $pasta = $empresaData['emp_code'] . " - " . $empresaData['emp_name'];
        $empresaData['pasta'] = $pasta;
        
        // Campos para insert (excluindo botões e campos não mapeados)
        ...
        $sql = "INSERT INTO empresas (" . implode(', ', $campos) . ", data) 
                VALUES (" . implode(', ', $placeholders) . ", NOW())";

        try {
            // Iniciar transação - isso garantirá que podemos reverter o cadastro se o e-mail falhar
            ...
            if ($resultado) {
                ...
                $emailClassPath = __DIR__ . '/emailEmpresa.php';
                $emailEnviado = false;
                ...
                if (file_exists($emailClassPath)) {
                    try {
                        ...
                        $emailEmpresa = new EmailEmpresa($conn);
                        
                        // Enviar e-mail - isso lançará uma exceção se falhar
                        $emailEmpresa->enviarEmailCadastro($empresaData);
                        
                        // Se chegou aqui, o e-mail foi enviado com sucesso
                        $emailEnviado = true;
                    } catch (Exception $e) {
                        // Registrar erro no log, mas continuar com o cadastro
                        error_log("Aviso: Não foi possível enviar e-mail de notificação: " . $e->getMessage());
                        // Não interromper o fluxo de cadastro por causa do e-mail
                    }
                } else {
                    ...
                    // Criar diretório para documentos da empresa
                    $diretorio = __DIR__ . "/../../documentos/empresas/" . $pasta;
                    ...
                    // Mensagem de sucesso de acordo com o envio do e-mail
                    if ($emailEnviado) {
                        $success = "Empresa cadastrada com sucesso e e-mail de notificação enviado!";
                    } else {
                        $success = "Empresa cadastrada com sucesso! (Não foi possível enviar o e-mail de notificação)";
                    }
                    
                    // Registrar a ação no log
                    if (function_exists('Logger')) {
                        Logger::activity('empresa', "Empresa {$empresaData['emp_name']} (CNPJ: {$empresaData['emp_cnpj']}) foi cadastrada");
                    }
                    ...
                    $empresaData['emp_sit_cad'] = 'ATIVA';
                    $empresaData['emp_reg_apu'] = 'SIMPLES';
                    $empresaData['emp_porte'] = 'ME';
                    $empresaData['emp_tipo_jur'] = 'LTDA';
                    
                // Não há mais catch para e-mail aqui, pois tratamos o e-mail de forma que não interrompa o fluxo
            } else {
                // Desfazer transação em caso de falha na inserção
                $conn->rollBack();
                
                $error = "Não foi possível cadastrar a empresa no banco de dados.";
            }
            ...
            error_log("Erro PDO ao cadastrar empresa: " . $e->getMessage());
        } catch (Exception $e) {
            ...
            $error = "Erro durante o cadastro: " . $e->getMessage();
            error_log("Erro geral ao cadastrar empresa: " . $e->getMessage());
        }
    } catch (PDOException $e) {
        $error = "Erro de banco de dados: " . $e->getMessage();
    ...
    // Adicionar debug para desenvolvimento
    //if (isset($error) && !empty($error)) {
    //    $debugInfo = "<div class='alert alert-warning mt-3'>
    //        <h5>Informações de Debug:</h5>
    //        <p>Erro: " . htmlspecialchars($error) . "</p>";
    //    
    //    if (isset($sql)) {
    //        $debugInfo .= "<p>SQL: " . htmlspecialchars($sql) . "</p>";
    //    }
    //    
    //    if (isset($stmt) && method_exists($stmt, 'errorInfo') && $stmt->errorInfo()[2]) {
    //        $debugInfo .= "<p>SQL Error: " . htmlspecialchars(implode(' ', $stmt->errorInfo())) . "</p>";
    //    }
    //    
    //    $debugInfo .= "</div>";
    //}
                            ...
                            <form action="" method="post" enctype="multipart/form-data" id="empresaForm" class="needs-validation" novalidate>
                                ...
                                <div class="tab-content" id="empresaTabsContent">
                                    ...
                                    <div class="tab-pane fade show active" id="dados" role="tabpanel" aria-labelledby="dados-tab">
                                        ...
                                        <div class="form-section">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-md-6">
                                                    <label for="emp_reg_nire" class="form-label">NIRE/Protocolo</label>
                                    ...
                                    <div class="tab-pane fade" id="certificado" role="tabpanel" aria-labelledby="certificado-tab">
                                        <div class="form-section">
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-md-6">
                                                    <label for="emp_cod_ace" class="form-label">Código de Acesso Simples Nacional (Caso Seja)</label>
                                                ...
                                                <div class="col-md-6">
                                                    <label for="emp_cod_pre" class="form-label">Senha Prefeitura</label>
                                ...
                                <div class="tab-pane fade" id="documentos" role="tabpanel" aria-labelledby="documentos-tab">
                                    <div class="form-section">
                                        ...
                                        <div class="alert alert-info mb-4">
                                            <i class="fas fa-info-circle me-2"></i> Faça o upload dos documentos da empresa. Formatos aceitos: PDF, JPG, PNG (máx. 5MB por arquivo). OPCIONAL
                                        </div>
...
<script>
// Configurar o modal de visualização
$('.view-empresa').click(function() {
    ...
    $('#editar-empresa-btn').attr('href', '/ged2.0/views/empresas/edit.php?id=' + id);
    ...
    $.ajax({
        url: '/ged2.0/controllers/empresas_controller.php',
        type: 'GET',
        data: {
            acao: 'obter',
            id: id,
        ...
        success: function(response) {
            if (response.success) {
                ...
                let html = `
                    <div class="row">
                        <div class="col-md-6">
                            <h4 class="mb-3">${empresa.emp_name}</h4>
                            <p><strong>Código:</strong> ${empresa.emp_code}</p>
                            <p><strong>CNPJ:</strong> ${formatarCnpj(empresa.emp_cnpj)}</p>
                            <p><strong>Responsável:</strong> ${empresa.name || 'Não informado'}</p>
                            <p><strong>Situação:</strong> <span class="badge status-badge status-${empresa.emp_sit_cad.toLowerCase()}">${getSituacaoCadastral(empresa.emp_sit_cad)}</span></p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Tipo:</strong> ${getTipoJuridico(empresa.emp_tipo_jur)}</p>
                            <p><strong>Porte:</strong> ${empresa.emp_porte || 'Não informado'}</p>
                            <p><strong>Localização:</strong> ${empresa.emp_cid}/${empresa.emp_uf}</p>
                            <p><strong>Data de Cadastro:</strong> ${formatarData(empresa.data)}</p>
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="text-end mt-2">
                                <a href="/ged2.0/views/documentos/list.php?empresa_id=${empresa.id}" class="btn btn-sm btn-outline-primary">
                                    <i class="fas fa-folder-open me-1"></i> Ver Todos os Documentos
                                </a>
                            </div>
                        </div>
                    </div>
                ...
                $('#empresa-detalhes').html(html).show();
                $('.spinner-border').hide();
                
                // Carregar documentos recentes (opcional)
                carregarDocumentosRecentes(id);
            } else {
                $('#empresa-detalhes').html('<div class="alert alert-danger">Erro ao carregar detalhes da empresa: ' + response.message + '</div>').show();
                $('.spinner-border').hide();
            }
        },
        error: function() {
            $('#empresa-detalhes').html('<div class="alert alert-danger">Erro ao conectar ao servidor. Tente novamente mais tarde.</div>').show();
            $('.spinner-border').hide();
        }
    });
});

// Função para formatar CNPJ no JavaScript
...
// Função para formatar data no JavaScript
...
function carregarDocumentosRecentes(empresaId) {
    $.ajax({
        url: '/ged2.0/controllers/empresasController.php',
        type: 'GET',
        data: {
            acao: 'listar_recentes',
        ...
        success: function(response) {
            if (response.success && response.data.length > 0) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="/ged2.0/controllers/documentos_controller.php?acao=download&id=${doc.id}" 
                                       class="btn btn-sm btn-outline-primary" title="Download">
                                        <i class="fas fa-download"></i>
                                    </a>
                                    <a href="/ged2.0/views/documentos/view.php?id=${doc.id}" 
                ...
                $('#documentos-recentes').html(html);
            }
        },
        error: function() {
            $('#documentos-recentes').html('<tr><td colspan="4" class="text-center text-danger">Erro ao carregar documentos</td></tr>');
        }
    });
}
</script>
